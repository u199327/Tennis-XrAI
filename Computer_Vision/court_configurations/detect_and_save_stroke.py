...
def add_data_to_video(input_video, court_detector, players_detector, ball_detector, strokes_predictions, skeleton_df_1, skeleton_df_2,
                      statistics, show_video, with_frame, output_folder, output_file, p1, p2, f_x, f_y):
    """
    ...
    # Initialize Matplotlib figure if show_video is True
    if show_video:
        fig, ax = plt.subplots()
    ...
    while True:
        ...
        for i in range(-10, 10):
            if frame_number + i in strokes_predictions:
                probs, stroke = strokes_predictions[frame_number + i]['probs'], strokes_predictions[frame_number + i]['stroke']
                cv2.putText(img, f'Forehand - {probs[0]:.2f}, Backhand - {probs[1]:.2f}, Service - {probs[2]:.2f}',
                            (70, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        ...
        cv2.putText(img, f'Distance: {player1_dists[frame_number] / 100:.2f} m',
                    (50, 500), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(img, f'Distance: {player2_dists[frame_number] / 100:.2f} m',
                    (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
    ...
    print(f'Creating new video frames {length}/{length}\n')
    print(f'New video created: {output_file}.avi')
...
def video_process(video_path, show_video=False, include_video=True,
    ...
    add_data_to_video(input_video=video_path, court_detector=court_detector, players_detector=detection_model,
                      ball_detector=ball_detector, strokes_predictions=predictions, skeleton_df_1=df_smooth_1,
                      skeleton_df_2 = df_smooth_2,statistics=statistics,
