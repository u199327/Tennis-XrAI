...
class PoseExtractor:
    ...
    def save_to_csv(self, output_folder, filename="stickman_data.csv"):
        """
        Saves the pose keypoints data as a CSV file.
        
        :param output_folder: str, path to output folder
        :param filename: str, name of the output CSV file (default: 'stickman_data.csv')
        :return: df, the DataFrame of the pose keypoints
        """
        ...
        # Ensure self.data has the correct number of columns
        '''
        if len(self.data[0]) != len(columns_x) + len(columns_y):
            raise ValueError(f"Data shape mismatch: expected {len(columns_x) + len(columns_y)} columns, but got {len(self.data[0])}.")
        '''
