...
def find_strokes_indices(player_1_boxes, player_2_boxes, ball_positions, skeleton_df, verbose=0):
    ...
    #print("Player 2 centers: ", player_2_centers)
...
def find_strokes_indices_v2(player_1_boxes, player_2_boxes, ball_positions, skeleton_df_1, skeleton_df_2, verbose=0):
    ...
    # Get top player wrists positions
    left_wrist_index = 9
    right_wrist_index = 10
...
def add_data_to_video(input_video, court_detector, players_detector, ball_detector,
    ...
    while True:
        ...
        # Add court, player boxes, and ball location
        ...
        # Show stroke predictions for both players
        ...
        # Show stroke detection highlight
        for i in range(-5, 10):
...
def append_strokes_to_csv(input_file, smoothed_csv, stroke_indices, output_csv, fps):
    ...
    # Filter rows that match stroke indices
    df_filtered = df_smooth.iloc[stroke_indices].copy()

    # Add new columns
    df_filtered.insert(0, "Filename", input_file)
    df_filtered.insert(1, "Strike_Second", df_filtered.index / fps)  # Convert index to seconds
...
# Function to extract features and predict strokes
def predict_strokes(rf_clf,label_encoder ,df_smooth, stroke_indices, player_name):
    if df_smooth is not None:
        ...
        for index in stroke_indices:
            if index < len(df_smooth):  # Ensure index is within bounds
                skeleton_features = df_smooth.iloc[index].values.reshape(1, -1)  # Convert to 2D array
                prediction = rf_clf.predict(skeleton_features)[0]  # Predict class
                # Convert encoded prediction back to original label
                prediction_label = label_encoder.inverse_transform([prediction])[0]
                
                print(f"Frame {index}: {prediction_label}")

# Función para extraer características de una ventana de 5 frames y hacer la predicción mayoritaria
def predict_strokes_window(rf_clf,label_encoder ,df_smooth, stroke_indices, player_name, window_size=5):
    frame_predictions = {}  # Dictionary to store predictions

    if df_smooth is not None:
        
        for index in stroke_indices:
            # Obtener la ventana de frames alrededor del índice detectado
            start = max(0, index - window_size // 2)
            end = min(len(df_smooth), index + window_size // 2 + 1)

            predictions = []
            for i in range(start, end):
                skeleton_features = df_smooth.iloc[i].values.reshape(1, -1)  # Asegurar forma 2D
                prediction = rf_clf.predict(skeleton_features)[0]
                prediction_label = str(label_encoder.inverse_transform([prediction])[0])  # Convert to plain string

                predictions.append(prediction_label)

            # Elegir la predicción más frecuente en la ventana
