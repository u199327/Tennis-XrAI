...
class CourtDetector:
    ...
    def get_warped_court(self):
        ...
        """
        # Ensure warp matrix is 3x3 and of correct type
        if self.court_warp_matrix[-1].shape != (3, 3):
            raise ValueError("Warp matrix must be 3x3, but got shape: " + str(self.court_warp_matrix[-1].shape))
        
        if self.court_warp_matrix[-1].dtype not in [np.float32, np.float64]:
            print("Converting warp matrix to float32")
            self.court_warp_matrix[-1] = self.court_warp_matrix[-1].astype(np.float32)

        # Apply perspective transformation
        court = cv2.warpPerspective(self.court_reference.court, self.court_warp_matrix[-1], self.frame.shape[1::-1])

        print("Warped court shape:", court.shape)
