...
class ThetisDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, transform=None, train=True, use_features=True, three_classes=True,
        ...
        # Si csv_file es una ruta de archivo (string), cargarlo como DataFrame
        if isinstance(csv_file, str):  
            self.videos_name = pd.read_csv(csv_file)
        else:
            self.videos_name = csv_file  # Si ya es un DataFrame, usarlo directamente
...
if __name__ == '__main__':
    ...
    rootdir = Path('dataset/VIDEO_RGB/')  # Adjust as needed
    ...
    for folder in rootdir.iterdir():
        if folder.is_dir():
            for file in folder.iterdir():
                if file.is_file():
                    data.append([folder.name, file.name])
                    print([folder.name, file.name])

    # Save as CSV
    df = pd.DataFrame(data, columns=['folder', 'name'])
    outfile_path = rootdir / 'THETIS_data.csv'
